...
/// ex 1.1
// fractal patrat (3x3 fara centru)
void fractalRecursiveSquares(double cx, double cy, double size, int recursionsLeft) {
    // deseneaza patratul curent (centrat in (cx, cy))
    double x = cx - size / 2.0;
    double y = cy - size / 2.0;

    Turtle t(x, y);
    t.resetRotation();
    drawSquare(t, (float)size);

    if (recursionsLeft <= 0) return;

    int next = recursionsLeft - 1;

    double offset = size;
    double child = size / 3.0;

    for (int dy = -1; dy <= 1; ++dy) {
        for (int dx = -1; dx <= 1; ++dx) {
            if (dx == 0 && dy == 0) continue; // skip centru
            fractalRecursiveSquares(cx + dx * offset, cy + dy * offset, child, next);
        }
    }
}
...
/// ex 1.2
// fractal hexagon
void fractalHexLine(Turtle t, float distance, int recursionsLeft, bool invert = false) {
    if (recursionsLeft > 0) {
        --recursionsLeft;
        distance /= 2;

        t.rotate((invert ? -1 : 1) * pi / 3);
        fractalHexLine(t, distance, recursionsLeft, !invert);
        t.move(distance);
...
void Display3() {
    ...
    fractalRecursiveSquares(0.0, 0.0, 0.6, g_recursionCurrent);
}
...
void Display4() {
    ...
    float size = 1.9;
    Turtle t(-0.90, -0.90);
    fractalHexLine(t, size, g_recursionCurrent);
}
...
protected:
    ...
    void colorMap(int iteration)
    {
        const double colors[14][3] = {
            {255,0,0},{238,34,17},{221,68,34},{204,102,51},
            {187,136,68},{170,170,85},{153,204,102},{119,238,136},
            {102,204,153},{85,170,170},{68,136,187},{51,102,204},
            {34,68,221},{17,34,238}
        };
        ...
        if (iteration == 0)
            ...
            int idx = (iteration - 1) % 14;
            glColor3f(colors[idx][0] / 255.0,
                colors[idx][1] / 255.0,
                colors[idx][2] / 255.0);
        }
    }
...
public:
    MB(FloatType xmin, FloatType xmax, FloatType ymin, FloatType ymax,
        FloatType maxRadius = 2, int maxIteration = 150)
        : JF<FloatType>(xmin, xmax, ymin, ymax, 0, 0, maxRadius, maxIteration) {}

protected:
    // Mandelbrot: c = punctul, z0 = 0
    inline int test(std::complex<FloatType> point, std::complex<FloatType> /*unused*/,
        double maxRadius = 2, int maxIteration = 50) override
    {
        std::complex<FloatType> c = point;
        std::complex<FloatType> z(0, 0);   // IMPORTANT: z0 = 0

        for (int ii = maxIteration; ii > 0; --ii) {
            z = z * z + c;
            if (std::abs(z) > maxRadius)
                return ii; // a diverged, return iterations left
        }
        return 0; // inside set (didn't diverge)
    }

public:
    // Ex 2.1: doua culori (rosu/alb)
    void drawTwoColor(FloatType l, FloatType r, FloatType b, FloatType t,
        int samplePointsHorizontal, int samplePointsVertical)
    {
        glPointSize(1);
        ...
        for (FloatType jj = 0, y = this->m_ymin, v = b; jj < samplePointsVertical; jj += 1, y += stepy, v += stepv) {
            ...
            for (FloatType ii = 0, x = this->m_xmin, h = l; ii < samplePointsHorizontal; ii += 1, x += stepx, h += steph) {
                ...
                int itLeft = test(point, this->m_c, this->m_maxRadius, this->m_maxIteration);

                if (itLeft == 0) {
                    glColor3f(1, 0, 0);   // inside = rosu
                }
                else {
                    glColor3f(1, 1, 1);   // outside = alb
                }
                glVertex2d(h, v);
            }
        }
    ...
    // Ex 2.2: colorat (foloseste colorMap din JF)
    void drawColored(FloatType l, FloatType r, FloatType b, FloatType t,
        int samplePointsHorizontal, int samplePointsVertical)
    {
        glPointSize(1);
        ...
        for (FloatType jj = 0, y = this->m_ymin, v = b; jj < samplePointsVertical; jj += 1, y += stepy, v += stepv) {
            ...
            for (FloatType ii = 0, x = this->m_xmin, h = l; ii < samplePointsHorizontal; ii += 1, x += stepx, h += steph) {
                ...
                // colorMap(0) => negru (inside), altfel paleta (outside)
                JF<FloatType>::colorMap(itLeft);
                glVertex2d(h, v);
            }
        }
        glEnd();
...
void Display6() {
    float drawSize = 1.0f;
    MB<double> mb(-2, 2, -2, 2, 2, 150);

    mb.drawTwoColor(-drawSize, drawSize, -drawSize, drawSize, g_w + 1, g_h + 1);
}
...
void Display7() {
    ...
    mb.drawColored(-drawSize, drawSize, -drawSize, drawSize, g_w + 1, g_h + 1);
}
